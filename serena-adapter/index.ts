...
class SerenaAdapter {
   ...
   */
  /**
   * Inserts a block of code into a file within a task's workspace.
   * This is a simple mock that appends to the file.
   * @param taskId - The ID of the task, used to find the workspace.
   * @param filePath - The relative path within the workspace.
   * @param code - The code to insert.
   */
  async insert_code(taskId: string, filePath: string, code: string): Promise<void> {
    const workspacePath = workspaceManager.getPath(taskId);
    const absoluteFilePath = path.join(workspacePath, filePath);
    logger.info(`[SerenaAdapter] Inserting code into '${filePath}' for task ${taskId}`);

    await fs.appendFile(absoluteFilePath, `\\n${code}`);
  }

  /**
   * Replaces a block of code in a file within a task's workspace.
   * This is a simple mock that replaces the entire file content.
   * @param taskId - The ID of the task, used to find the workspace.
   * @param filePath - The relative path within the workspace.
   * @param content - The new content for the file.
   */
  async replace_code_block(taskId: string, filePath: string, content: string): Promise<void> {
    const workspacePath = workspaceManager.getPath(taskId);
    const absoluteFilePath = path.join(workspacePath, filePath);
    logger.info(`[SerenaAdapter] Replacing content of '${filePath}' for task ${taskId}`);

    await fs.writeFile(absoluteFilePath, content, 'utf8');
  }

  /**
   * Creates a new file in the task's workspace.
   * @param taskId - The ID of the task.
   * @param filePath - The relative path for the new file.
   * @param content - The initial content of the file.
   */
  async create_file(taskId: string, filePath: string, content: string): Promise<void> {
    const workspacePath = workspaceManager.getPath(taskId);
    const absoluteFilePath = path.join(workspacePath, filePath);
    logger.info(`[SerenaAdapter] Creating new file '${filePath}' for task ${taskId}`);
